...
Using this notation, we write our stiffness matrix as the sum of the Toeplitz blocks, blocks adjacent to the diagonal, and blocks further removed from the diagonal, $\lowright = \toediag + \offd + \offoff$ (see Figure (\ref{fig:split})). 
...
\begin{figure}[h]
	...
	\end{tikzcd}
	\caption{Illustration of the splitting of $T$: $\toediag$ is block diagonal with Toeplitz blocks, blocks in $\offd$ are assumed to be well-approximated by low-rank matrices, and $\offoff$ is assumed to be small-norm due to weak interaction between elements.  }
	\label{fig:split}
\end{figure}
...
The cost of applying $\precond^{-1}$ is dominated by application of the biggest block, $\hat{\Schur}^{-1}$. However, by using $\bigpre$ in place of $\lowright$, and consequently $\hat{\Schur}$ in place of $\Schur$, this can be done in $\bigOh(m \log m)$ FLOPS, as explained above. Therefore $\precond$ also satisfies Criteria (\ref{crit1}) and (\ref{crit2}). 
...
Proofs demonstrating the eigenvalue clustering of a circulant or trigonometric-based preconditioners on a Toeplitz matrices can follow a few strategies, here we briefly give our version, which utilizes the infinite matrix framework set forth in Section (\ref{sec:matrix_props}). Proofs in \cite{Bini} and \cite{Kailath} follow a similar technique, see \cite{Chan_Strang} for an approach relying on the infinite matrix framework and theory of collectively compact operators. 
...
So the clustering depends on the spectrum of $I + \bigpre^{-1} \SN + \bigpre^{-1} \LR + ( \bigpre^{-1}\upright^{\top}(\upleft-\upright\bigpre^{-1}\upright^{\top})^{-1}\upright)(\bigpre^{-1} \SN + \bigpre^{-1} \LR)$. Most of the non-identity terms here are easily characterized using the previous results as either low-rank or small-norm. However, we must assume $(\bigpre^{-1}\upright^{\top}(\upleft-\upright\bigpre^{-1}\upright^{\top})^{-1}\upright)\bigpre^{-1} \SN$ is small-norm. Numerical validation of this assumption is found in Section (\ref{validate}). 
\end{proof}
